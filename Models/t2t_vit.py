...
class T2T_module(nn.Module):
    ...
    def __init__(self, img_size=224, tokens_type='transformer', in_chans=3, embed_dim=768, token_dim=64, method=None):
        ...
        # self.num_patches = (img_size // (4 * 2 * 2)) * (img_size * 2 // (4 * 2 * 2)) * 2  # there are 3 sfot split, stride are 4,2,2 seperately
        self.num_patches = (img_size // (4 * 2 * 2)) * (img_size // (4 * 2 * 2))  # todo
    def forward(self, x):
        # step0: soft split
