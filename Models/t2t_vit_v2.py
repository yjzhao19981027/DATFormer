...
from .transformer_block import Block, dBlock, get_sinusoid_encoding
...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 1000, 'input_size': (3, 224, 448), 'pool_size': None,
...
class T2T_module(nn.Module):
    ...
    """
    def __init__(self, img_size=224, tokens_type='transformer', in_chans=3, embed_dim=768, token_dim=64, method=None):
        super().__init__()
        self.method = method
        self.surface_num = 1
        ...
        self.num_patches = (img_size // (4 * 2 * 2)) * (img_size * 2 // (4 * 2 * 2))  # there are 3 sfot split, stride are 4,2,2 seperately
    ...
    def forward(self, x):
        ...
        x = x_1_4.transpose(1,2).reshape(B, C, int(np.sqrt(new_HW // 2)), int(np.sqrt(new_HW // 2) * 2))
        ...
        x = x_1_8.transpose(1, 2).reshape(B, C, int(np.sqrt(new_HW // 2)), int(np.sqrt(new_HW // 2) * 2))
...
class T2T_ViT(nn.Module):
    def __init__(self, img_size=224, tokens_type='performer', in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., norm_layer=nn.LayerNorm, method=None):
        super().__init__()
        self.num_classes = num_classes
        self.num_features = self.embed_dim = embed_dim  # num_features for consistency with other models

        self.tokens_to_token = T2T_module(
                img_size=img_size, tokens_type=tokens_type, in_chans=in_chans, embed_dim=embed_dim, method=method)
    ...
    def forward_features(self, x, density_map=None):
        B = x.shape[0]
        x, x_1_8, x_1_4 = self.tokens_to_token(x)

        if not density_map == None:
            _, C, H, W = density_map.shape
            density_map = density_map.reshape(B, C, -1).transpose(1, 2)
            x = x + density_map
            # pos_embed = self.pos_embed.expand(B, -1, -1)
            # pos_embed_head = pos_embed[:, 0, :].unsqueeze(1)
            # pos_embed_body = pos_embed[:, 1:, :] * density_map
            # pos_embed = torch.cat((pos_embed_head, pos_embed_body), dim=1)
    ...
    def forward(self, x, density_map=None):
        x, x_1_8, x_1_4 = self.forward_features(x, density_map)
        # x = self.head(x)
        return x, x_1_8, x_1_4


@register_model
def T2t_vit_t_14_v2(pretrained=False, **kwargs):  # adopt transformers for tokens to token
    # if pretrained:
    ...
    model = T2T_ViT(tokens_type='transformer', embed_dim=384, depth=14, num_heads=6, mlp_ratio=3., method=args.methods)
    # model = T2T_ViT(tokens_type='transformer', embed_dim=384, depth=14, num_heads=6, mlp_ratio=3., drop_rate=0.5, method=args.methods)
    model.default_cfg = default_cfgs['T2t_vit_t_14']
    if pretrained:
        model.load_state_dict(torch.load(args.pretrained_model_v2), strict=False)
        # load_checkpoint(model, args.pretrained_model, use_ema=True)
        print('Model loaded from {}'.format(args.pretrained_model_v2))
        # load_pretrained(
