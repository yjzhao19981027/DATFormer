...
from .DConvMLP import DeformableCNN, DConvMLP
...
class dMlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        self.adapter_dcnn = DeformableCNN(in_features, in_features, (3, 3), 1, 0)
    ...
    def forward(self, x):
        ...
        W = int(H * 2)
        x2 = x.reshape(B, H, W, C).permute(0, 3, 1, 2)
        x = x + self.adapter_dcnn(x2).permute(0, 2, 3, 1).reshape(B, HW, -1)
        x = self.fc(x)
        x = self.act(x)
        x = self.drop(x)
        x = self.fc2(x)
...
class dBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, time_chans=1):
        super().__init__()
        ...
        self.mlp = dMlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
...
class TimeSpatialBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
            drop_path=0.1, act_layer=nn.GELU, norm_layer=nn.LayerNorm, attention_type="parallel"):
        super().__init__()
        ...
        if attention_type == "parallel":
            self.temporal_spatial_project = nn.Linear(dim * 2, dim)
    ...
    def forward(self, x):
        # todo TimeSpatialAttention
        # x [B, T, HW, C]
        B, T, _, C = x.shape

        # Temporal
        x_temporal = x[:, :, 1:, :]
        x_temporal = x_temporal.permute(0, 2, 1, 3).reshape(-1, T, C)
        res_temporal = self.drop_path(self.temporal_attn(self.temporal_norm1(x_temporal)))
        res_temporal = res_temporal.reshape(B, -1, T, C).permute(0, 2, 1, 3)
        res_temporal = self.temporal_fc(res_temporal)
        x_temporal = x[:, :, 1:, :] + res_temporal

        # Spatial
        if self.attention_type == "serial":
            cls_token = x[:, :, 0, :].unsqueeze(2).reshape(B * T, 1, -1)
            x_spatial = x_temporal.reshape(B*T, -1, C)
            x_spatial = torch.cat((cls_token, x_spatial), dim=1)
            res_spatial = self.drop_path(self.attn(self.norm1(x_spatial)))
            res_spatial = res_spatial.reshape(B, T, -1, C)
            cls_token = cls_token.reshape(B, T, 1, -1)

            x = res_spatial + torch.cat((cls_token, res_temporal), dim=2)
        elif self.attention_type == "parallel":
            cls_token = x[:, :, 0, :].unsqueeze(2)
            x_spatial = x.reshape(B*T, -1, C)
            res_spatial = self.drop_path(self.attn(self.norm1(x_spatial)))
            res_spatial = res_spatial.reshape(B, T, -1, C)

            x = torch.cat((res_spatial, torch.cat((cls_token, res_temporal), dim=2)), dim=3)
            x = self.temporal_spatial_project(x)
...
class DeformableBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = DConvMLP(in_chans=dim, emb_chans=mlp_hidden_dim, act_layer=act_layer, drop=drop, time_chans=time_chans)
